...
class CheckWm extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      summary: '',
  ...
  componentDidMount() {
    ...
    let ids = /entities\/check\/person\/([0-9]+)/g.exec(pathname);
    if (ids) {
      let id = ids[1];
      console.log(id);
  ...
  changeMultiChecked(check) {
    ...
    keys = getFieldValue(`occupationInfoKey`);
    for (let k of keys) {
      setFieldsValue({ [`occupationInfo[${k}].checked`]: check });
    }
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      if (err || isEmpty(summary)) {
        ...
        for (let k of occupationInfoKey) {
          ...
          let ostatus;
          if (!occupationInfo[k].checked) {
            status = ostatus = -1;
          } else {
            ostatus = oname == data.occupations[k].oname ? 1 : 2;
          }
          let eoid = data.occupations[k].eoid;
          occupations.push({ oid, oname, ostatus, eoid });
        }
        ...
        //manager
        if (button && status != -1 && this.state.role == 1) {
          status = 3;
        }
        if (!photo) {
          photo = undefined;
...
CheckWm.contextType = LayoutContext;
const WrappedCheckWm = Form.create({ name: 'checkWm' })(CheckWm);

export default WrappedCheckWm;
