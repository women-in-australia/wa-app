...
import BaseInformation from '../../containers/CreateOgEntity/baseInformation';

const { TextArea } = Input;

class OgDraft extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      summary: '',
  ...
  componentDidMount() {
    ...
    if (/\/reject/.test(pathname)) {
      ids = /entities\/reject\/organi[sz]ation\/([0-9]+)/g.exec(pathname);
    } else {
      ids = /entities\/draft\/organi[sz]ation\/([0-9]+)/g.exec(pathname);
    }
    if (ids) {
      let id = ids[1];
      console.log(id);
...
OgDraft.contextType = LayoutContext;
const WrappedOgDraft = Form.create({ name: 'ogDraft' })(OgDraft);

export default WrappedOgDraft;
